...
export default function certificationsPage() {
  ...
  const [drafts, setDrafts] = useState<Record<string, Partial<certification>>>({});
  ...
  const fetchcertifications = async () => {
    ...
    try {
      const url = `/api/certifications?page=${page}&limit=${pageSize}&q=${encodeURIComponent(query)}&t=${Date.now()}`;
      ...
      if (json?.success) {
        const d = json.data || {};
        setcertifications(Array.isArray(d.items) ? d.items : []);
  ...
  const startEdit = (b: certification) => {
    setEditingId(b._id);
    setDrafts((d) => ({ ...d, [b._id]: { ...b } }));
  };

  const updateDraft = (id: string, change: Partial<certification>) => {
    setDrafts((d) => ({
      ...d,
      [id]: { ...d[id], ...change },
    }));
  ...
  const toggleActive = async (id: string, newActive: boolean) => {
    ...
    setcertifications((bs) => bs.map((b) => (b._id === id ? { ...b, active: newActive } : b)));

    try {
      const res = await fetch(`/api/certifications/${id}?t=${Date.now()}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
    ...
    <div className="p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage your certification posts</p>
        </div>
      ...
      <div className="bg-white border rounded-lg shadow-sm">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-400">No certifications found.</div>
        ) : viewMode === 'grid' ? (
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {certifications.map((b) => (
              <CertificationCard key={b._id} certification={b} onDelete={handleDelete} />
            ))}
          </div>
        ) : (
          /* Table View */
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {certifications.map((b) => {
                  ...
                  const draft = drafts[b._id] as certification | undefined;
                      ...
                      <td className="px-6 py-4 text-sm">
                        {isEditing ? (
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            <Link href={`/admin/certifications/${b._id}`} className="text-blue-600 hover:underline">
                              Edit
                            </Link>
      ...
      <div className="flex items-center justify-between mt-6">
        <p className="text-sm text-gray-600">{total} total certifications</p>
